...
const FilterSection = ({filterOpen,fetchFilterReports}) => {
  ...
  return (
    <div className={` ${filterOpen ? 'w-3/10 max-w-full duration-300' : 'w-0 max-w-0 duration-300'} overflow-x-hidden shadow-lg border-s border-gray-200`}>
          <div className='flex justify-between items-center p-3'><div>Filters</div><div className='text-xs text-blue-800' onClick={()=>setIndex(-1)}>Reset All</div></div>
          <div className=' flex flex-col overflow-y-auto text-xs'>
            {CostExploreList.map((item,i) => {
              return <Filter fetchFilterReports={fetchFilterReports} key={item} item={item} index={i} setIndex={setIndex} open={index===i}/>
            })}
          </div>
        </div>
  )
